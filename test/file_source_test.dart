...
void main() {
  ...
  group(
    'a picture is recognised as one, so it can be read rather than refused',
    () {
      ...
      test(
        'JPEG, which is what it becomes once it has been through a chat app',
        () {
          expect(
            looksLikeImage(bytes([0xff, 0xd8, 0xff, 0xe0, 0, 16])),
            isTrue,
          );
        },
      );
      ...
      test('the exports Wren actually reads are NOT taken for pictures', () {
        ...
        expect(
          looksLikeImage(bytes(utf8.encode('<?xml version="1.0"?><kml/>'))),
          isFalse,
        );
        expect(
          looksLikeImage(bytes(utf8.encode('{"type":"FeatureCollection"}'))),
          isFalse,
        );
